...
class FromZeroAppContentWrapper extends ConsumerStatefulWidget {
  ...
  static void exitApp(int code) {
    ...
    if (!kIsWeb && Platform.isWindows) {
      ...
      if (kReleaseMode && windowsProcessName != null) {
        log(
          LgLvl.fine,
          'Running process: taskkill /IM "$windowsProcessName" /F',
          type: FzLgType.routing,
        );
  ...
  static void _exit(int code) {
    log(
      LgLvl.fine,
      'Exiting the normal dart way (debugger(); + exit(0);)...',
      type: FzLgType.routing,
    );
    debugger();
    exit(code);
  ...
  void initState() {
    ...
    if (!kIsWeb && Platform.isWindows) {
      ...
      if (WindowEventListenerWindowManagerPackage.listener == null) {
        WindowEventListenerWindowManagerPackage.initListener(
          () => widget.goRouter,
        );
        windowManager.addListener(
          WindowEventListenerWindowManagerPackage.listener!,
        );
      }
    }
  }
          ...
          if (constraints.maxWidth > 0) {
            ...
            if (scaffoldChangeNotifier.previousWidth == null) {
              ...
              WidgetsBinding.instance.addPostFrameCallback((timeStamp) {
                ...
                scaffoldChangeNotifier.validateDrawerWidths(
              ...
              child: FittedBox(
                child: SizedBox(
                  ...
                  child: MediaQuery(
                    ...
                    child: Stack(
                      children: [
                        SnackBarHostFromZero(child: widget.child),
...
class AppearOnMouseOver extends StatefulWidget {
  ...
  const AppearOnMouseOver({required this.child, this.appear = true, super.key});
      ...
      while (true) {
        ...
        if (await navigator.maybePop()) {
          ...
          if (previousGoRouteFromZero?.pageScaffoldId != newGoRouteFromZero?.pageScaffoldId) {
            ...
            log(
              LgLvl.finer,
              'New route is a different scaffold ID, stopping iteration...',
              type: FzLgType.routing,
            );
            return;
          }
          ...
          log(
            LgLvl.finer,
            'Successfully popped last route, exiting app...',
            type: FzLgType.routing,
          );
        ...
        log(
          LgLvl.finer,
          'Popped successfully, continuing popping iteration...',
          type: FzLgType.routing,
        );
      }
    } catch (e, st) {
...
class MoveWindowFromZero extends StatefulWidget {
  ...
  const MoveWindowFromZero({this.child, this.onDoubleTap, super.key});
  ...
  double getCurrentDrawerWidth(String pageScaffoldId) {
    if (!_currentDrawerWidths.containsKey(pageScaffoldId)) {
      ...
      if (previousWidth != null) {
        validateDrawerWidths(
          previousWidth! < ScaffoldFromZero.screenSizeMedium,
          previousWidth!,
        );
      }
      blockNotify = false;
...
class CloseConfirmDialog extends StatelessWidget {
  ...
  const CloseConfirmDialog({this.forceExitApp, super.key});
