...
class TableFromZeroState<T> extends ConsumerState<TableFromZero<T>> with TickerProviderStateMixin {
  ...
  Map<dynamic, List<ConditionFilter>> get conditionFilters =>
      widget.tableController?.conditionFilters ?? _conditionFilters;
  ...
  ValueNotifier<Map<dynamic, List<dynamic>>?> availableFilters = ValueNotifier(
    null,
  );
  ...
  void init({bool notifyListeners = true, bool isFirstInit = false}) {
    ...
    if (widget.tableController?.conditionFilters == null) {
      if (widget.tableController?.initialConditionFilters == null) {
        ...
        conditionFilters = Map.from(
          widget.tableController!.initialConditionFilters ?? {},
        );
        filtersAltered = true;
      }
    } else if (isFirstInit) {
  ...
  Future<void> initFilters(
    ...
    if (computeFiltersInIsolate ?? widget.computeFiltersInIsolate ?? rows.length > 200) {
      try {
        ...
        availableFiltersIsolateController = cancelable_compute.compute(
          getAvailableFilters,
          [
            widget.columns!.map(
          ...
          if (fieldAliases.isEmpty && daoAliases.isEmpty) {
            ...
            final sorted = result.sortedWith(
              (a, b) => defaultComparator(
                a,
                b,
                widget.columns?[key]?.defaultSortAscending ?? true,
              ),
            );
        ...
        log(
          LgLvl.warning,
          'TabelFromZero - Isolate creation for computing table filters failed. Computing synchronously...',
          e: e,
          st: st,
        );
        initFilters(
  ...
  static dynamic _sanitizeValueForIsolate(
    dynamic key,
    dynamic value, {
    ...
    if (value is List) {
      ...
      final newValue = _sanitizeValueForIsolate(
        key,
        value.value,
        fieldAliases: fieldAliases,
        daoAliases: daoAliases,
  ...
  static Future<Map<dynamic, List<dynamic>>> getAvailableFilters(
    List<dynamic> params, {
  ...
  Map<dynamic, bool?> _getColInitialValueFiltersExcludeAllElse() {
    ...
    return {
      for (final e in widget.columns!.keys)
        e:
            widget.tableController?.initialValueFiltersExcludeAllElse ??
            widget.columns![e]!.initialValueFiltersExcludeAllElse,
    };
  }

  static Map<dynamic, Map<Object?, bool>>? _getValidInitialFilters(
    List<dynamic> params,
  ) {
  ...
  Widget build(BuildContext context) {
    ...
    if (widget.waitForFirstFilterToRender && showFiltersLoading) {
      result = const SliverToBoxAdapter(
        child: SizedBox(height: 128, child: LoadingSign()),
      );
    } else {
      ...
      if (widget.enableFixedHeightForListRows && allFiltered.isNotEmpty) {
        result = SliverFixedExtentList(
          ...
          delegate: SliverChildBuilderDelegate(
            (BuildContext context, int i) => _getRow(
              context,
              allFiltered.isEmpty ? null : allFiltered[i],
              i,
              minWidth,
            ),
        ...
        header: Theme(
          data: theme.copyWith(
            scrollbarTheme: theme.scrollbarTheme.copyWith(
              crossAxisMargin: theme.scrollbarTheme.crossAxisMargin?.clamp(
                widget.footerStickyOffset,
                double.infinity,
              ),
            ),
          ),
  ...
  Widget _getRow(
    ...
    if (row == null) {
      ...
      if (row == headerRowModel) {
        return widget.headerRowBuilder?.call(
              context,
              headerRowModel!,
              minWidth,
            ) ??
  ...
  Widget _defaultRowBuilder(
    ...
    final String? rowDisabledReason = row == headerRowModel
        ? null
    ...
    Widget builder(BuildContext context, BoxConstraints? constraints) {
          ...
          : widget.rowActions
                .map(
                  (e) => e.copyWith(
                    ...
                    disablingError: e.disablingErrorGetter?.call(
                      context,
                      row as RowModel<T>,
                    ),
                    breakpoints:
                        e.breakpointsGetter?.call(
                          context,
                          row as RowModel<T>,
                        ) ??
                        e.breakpoints,
      ...
      for (final e in rowActions) {
        if (!rowActionStates.containsKey(e)) {
          rowActionStates[e] = e.getStateForMaxWidth(
      ...
      if (widget.exportPathForExcel != null) {
        rowActions = addExportExcelAction(
          context,
          actions: rowActions,
      ...
      Widget cellBuilder(BuildContext context, int j) {
                  ...
                  children: [
                    if (row.isExpandable)
                      SizedBox(
                        ...
                        child: IconButton(
                          splashRadius: 24,
                          onPressed: row.isFilteredInBecauseOfChildren
                              ? null
                              : () {
                                        ...
                                        if (row.onCheckBoxSelected != null) {
                                          if (row.onCheckBoxSelected!(
                                            ...
                                            if (row.depth == 0) {
                                              ...
                                              setState(
                                                () {},
                                              ); // nested rows need to set state for whole table, so parents can react to potential changes
                                            }
                                          }
                                        } else {
                                          bool result = false;
        ...
        Widget result = Container(
          ...
          padding: row == headerRowModel
              ? null
              : row.depth > 0 && j == 0
              ? widget.cellPadding.copyWith(
                  left: widget.cellPadding.left + row.depth * _depthPadding,
                )
              : widget.cellPadding,
          child: SizedBox(
            ...
            child: row == headerRowModel
                ...
                : widget.cellBuilder?.call(
                        context,
                        row as RowModel<T>,
                        colKey,
                        col,
                      ) ??
                      TableFromZeroState.defaultCellBuilder<T>(
                        context,
                        row as RowModel<T>,
                        colKey,
                        col,
                        getStyle(
                          ...
                          colKey,
                          col,
                          widget.rowStyleTakesPriorityOverColumn,
                          rowDisabledReason != null,
                        ),
      ...
      if (row == headerRowModel) {
        ...
        if (row.height != null) {
          result = SizedBox(
            height: row.height,
            child: OverflowBox(
              // hack to fix Appbar actions overflowing when rowHeight<40
      ...
      if (widget.horizontalDivider != null) {
        result = Column(
          mainAxisSize: MainAxisSize.min,
          crossAxisAlignment: CrossAxisAlignment.stretch,
          children: [result, widget.horizontalDivider!],
        );
      }
      result = FocusTraversalGroup(
        policy: ReadingOrderTraversalPolicy(),
    ...
    result = ClipRect(clipBehavior: Clip.hardEdge, child: result);
    return result;
  }
  ...
  Widget _buildRowGestureDetector({
    ...
    if (tooltip != null) {
      result = TooltipFromZero(message: tooltip, child: result);
    }
    result = Material(
      type: MaterialType.transparency,
  ...
  Widget? _wrapRowAddon(
    ...
    if ((row.rowAddonIsCoveredByScrollable ?? true) &&
        constraints != null &&
        minWidth != null &&
      ...
      return NotificationListener(
        ...
        child: SingleChildScrollView(
          controller: sharedController,
          scrollDirection: Axis.horizontal,
          child: SizedBox(width: minWidth, child: row.rowAddon),
        ),
      );
    }
    return row.rowAddon;
    ...
    if (showFiltersLoading) {
      return ValueListenableBuilder<Map<dynamic, List<dynamic>>?>(
        ...
        builder: (context, availableFilters, child) {
          return _defaultHeaderCellBuilder(
            context,
            row,
            colKey,
  ...
  Widget _defaultHeaderCellBuilder(
    ...
    Widget result = Align(
      alignment: alignment == TextAlign.center
          ? Alignment.center
          : alignment == TextAlign.left || alignment == TextAlign.start
        ...
        children: [
          AnimatedPadding(
            ...
            child: AutoSizeText(
              ...
              minFontSize: 15,
              overflowReplacement: name != compactName
                  ? null
                  : TooltipFromZero(
                      message: name,
    ...
    if (widget.exportPathForExcel != null) {
      colActions = addExportExcelAction(
        context,
        actions: colActions,
        exportPathForExcel: widget.exportPathForExcel!,
        tableController: widget.tableController ?? (TableController()..currentState = this),
  ...
  static Widget defaultCellBuilder<T>(
    BuildContext context,
    RowModel<T> row,
    dynamic colKey,
    ColModel<dynamic>? col,
    ...
    Widget result = AutoSizeText(
      ...
      overflowReplacement: TooltipFromZero(
        ...
        child: Text(
          message,
          style: (style ?? const TextStyle()).copyWith(fontSize: 15),
          textAlign: alignment,
          maxLines: autoSizeTextMaxLines,
          softWrap: autoSizeTextMaxLines > 1,
          overflow: autoSizeTextMaxLines > 1 ? TextOverflow.clip : TextOverflow.fade,
    ...
    if (backgroundColor != null) {
      ...
      if (applyDarker) {
        backgroundColor = Color.alphaBlend(
          backgroundColor.withValues(alpha: 0.965),
          Colors.black,
        );
      }
    }
  ...
  bool _shouldApplyDarkerBackground(
    ...
    if (widget.alternateRowBackgroundSmartly ?? allFiltered.length < 50) {
      if (index < 0 || index > allFiltered.length) {
        ...
        Color? previous = _getBackgroundColor(
          allFiltered[index - 1],
          colKey,
          isHeader,
        );
        if (previous != current) return false;
        return !_shouldApplyDarkerBackground(
          previous,
          allFiltered[index - 1],
          index - 1,
          colKey,
    ...
    if (isDisabled) {
      ...
      style = style!.copyWith(
        color: Color.alphaBlend(
          style.color!.withValues(alpha: 0.66),
          Theme.of(context).disabledColor,
        ),
      );
    }
  ...
  static ActionFromZero getOpenFilterPopupAction(
    ...
    return ActionFromZero(
      ...
      breakpoints: {0: ActionState.popup},
      onTap: (context) async {
        final result = await controller.currentState!._showFilterPopup(
          colKey,
          anchorKey: globalKey,
  ...
  static ActionFromZero? getClearAllFiltersAction({
    ...
    if (skipConditions ||
        (controller.columns != null &&
            controller.columns!.any(
              (key, value) => (value.filterEnabled ?? true),
            ))) {
      return ActionFromZero(
  ...
  void clearAllFilters({bool updateStateIfModified = true}) {
  ...
  void _setAllChildrenHasExpandableRows(
    RowModel<T> row,
  ...
  FilterResults<T> getFilterResults(
    List<RowModel<T>> rows, {
    dynamic skipColKey,
  }) {
    ...
    final result = FilterResults(filtered: filtered);
  ...
  bool _passesFilters(RowModel<T> row, {dynamic skipColKey}) {
    bool pass = true;
    for (final key in valueFilters.keys) {
      if (key != skipColKey) {
