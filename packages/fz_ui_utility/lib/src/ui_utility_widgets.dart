  ...
  Widget build(BuildContext context) {
    return SliverPadding(
      padding: EdgeInsets.symmetric(
        horizontal: MediaQuery.sizeOf(context).width < breakpoint ? 0 : padding,
      ),
      sliver: sliver,
    );
  }
...
class ExpandIconButtonState extends State<ExpandIconButton>
  ...
  void initState() {
    ...
    controlPanelAnimationController = AnimationController(
      duration: const Duration(milliseconds: 300),
      vsync: this,
    );
    controlPanelAnimationController.value = widget.value ? 1 : 0;
    _iconTurns = controlPanelAnimationController.drive(
      _halfTween.chain(CurveTween(curve: Curves.easeIn)),
    );
  }
...
class ReturnToTopButtonState extends ConsumerState<ReturnToTopButton> {
  ...
  Widget build(BuildContext context) {
    ...
    try {
      space =
          ref.watch(
            fromZeroScreenProvider.select((value) => value.isMobileLayout),
          )
          ? 16
          : 32;
    } catch (_) {}
...
class TextIcon extends StatelessWidget {
  ...
  const TextIcon(this.text, {super.key, this.width = 24, this.height = 24});
      ...
      child: Container(
        ...
        child: Center(
          child: Text(
            ...
            style: Theme.of(
              context,
            ).textTheme.bodyLarge!.copyWith(color: Theme.of(context).cardColor),
          ),
        ),
      ),
    );
...
class IconButtonBackground extends StatelessWidget {
  ...
  const IconButtonBackground({required this.child, super.key});
...
class InitiallyAnimatedWidget extends StatefulWidget {
  ...
  /// false: don't repeat, null: repeat once, true: repeate forever
  final bool reverse;
...
class InitiallyAnimatedWidgetState extends State<InitiallyAnimatedWidget>
  ...
  Future<void> startAnimation() async {
    if (widget.repeat == null) {
      ...
      await animationController.repeat(reverse: widget.reverse);
    } else {
      await animationController.forward();
    }
...
class KeepAliveMixinWidgetState extends State<KeepAliveMixinWidget>
    with AutomaticKeepAliveClientMixin<KeepAliveMixinWidget> {
  @override
...
class _TimedOverlayState extends State<TimedOverlay> {
  ...
  Widget build(BuildContext context) {
    ...
    return Stack(
      children: [
        widget.builder(context, elapsed: elapsed, remaining: remaining),
...
class ComposedIcon extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Stack(
      ...
      children: [
        ClipPath(
          clipper: IconBottomRightClipper(percentage: subIconSize * clipSize),
          child: icon,
        ),
        Positioned.fill(
          child: Center(
            child: FractionalTranslation(
              ...
              child: Transform.scale(scale: subIconSize, child: subicon),
            ),
          ),
        ),
      ],
...
class IconBottomRightClipper extends CustomClipper<Path> {
  final double percentage;
  IconBottomRightClipper({this.percentage = 0.5});
  @override
  Path getClip(Size size) {
    final result = Path();
