...
class Field<T extends Comparable> extends ChangeNotifier implements Comparable, ContainsValue<T> {
  ...
  /// used for reversing the field to default state when hidden, noly if invalidateNonEmptyValuesIfHiddenInForm==true, default null
  ...
  Future<bool> validateRequired(
    ...
    bool isRequired = false;
    if (value == null ||
        (value is String && (value! as String).isBlank) ||
        (value is ComparableList && (value! as ComparableList).isEmpty)) {
  ...
  static Widget defaultViewWidgetBuilder<T extends Comparable>(
    ...
    linkToInnerDAOs = linkToInnerDAOs && (field.value is DAO) && (field.value! as DAO).wantsLinkToSelfFromOtherDAOs;
    final onTap = linkToInnerDAOs ? () => (field.value! as DAO).pushViewDialog(context) : null;
    if (message == null) {
...
class FieldGroup {
  ...
  Map<String, Field> get props {
    return {
      ...fields,
      if (childGroups.isNotEmpty) ...childGroups.map((e) => e.props).reduce((value, element) => {...value, ...element}),
    };
  }
  ...
  Map<String, Field> get visibleFields =>
      Map<String, Field>.from(fields)..removeWhere((key, value) => value.hiddenInForm);
