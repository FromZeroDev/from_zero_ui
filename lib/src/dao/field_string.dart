...
class StringField extends Field<String> {
  ...
  static Widget buildDaoTextFormField(
    BuildContext context, {
    ...
    return TextFormField(
      ...
      onChanged: onChanged,
      inputFormatters: [
        if (maxLines != null) LineLimitTextFormatter(maxLines),
    ...
    for (int i = 0; i < characters.length; i++) {
      if (characters[i] == '\n' || characters[i] == '\r') {
        ...
        if (lineCount >= lineLimit!) {
          final newText =
              newValue.text.substring(0, i) + newValue.text.substring(i).replaceAll(RegExp(r'\r\n|\r|\n|/'), ' ');
          return TextEditingValue(
            text: newText,
            selection: newValue.selection,
          );
